...
class Canvas(QWidget):
    ...
    def mouseReleaseEvent(self, ev):
        if ev.button() == Qt.RightButton:
            ...
            self.current_brush_path = None
        elif ev.button() == Qt.LeftButton and self.task_mode == 4 and self.point_move:
    ...
    def paintEvent(self, event):#所有的绘制操作都在paintEvent中完成
        ...
        if self.task_mode==4:
            ...
            if self.point_dex :#注意这里剔除了0 0是none
                ...
                p.drawRect(float(self.point_point_list[self.point_dex - 1].x() - 2),
                           float(self.point_point_list[self.point_dex - 1].y() - 2), 4, 4)
                if self.point_move:
                    p.setPen(QColor(255,255,255))
