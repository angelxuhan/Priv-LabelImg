...
###################这部分代码开始加入整个布局了#####################
    ...
    def addCLSLabel(self,label):#添加分类标签
        ...
        item = QListWidgetItem(label)#QListWidgetItem 是QListWidgetItem中的item对象
    ...
    def labelitemDoubleClicked(self, item=None):
        ...
        if item:
            label = str(item.text())
            if label not in self.currentItemLabels:#如果当前的label不在当前的item中，则添加进去
...
#
    ...
    def saveFile(self, _value=False):#保存文件
        ...
        if self.hasLabels():#如果当前标注没有label，则return false
            ...
            #     os.remove(savedPath)
            print('saved path:',self.savedPath)
            if os.path.isfile(self.savedPath):
                print('remove path:', self.savedPath)
